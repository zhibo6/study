...
public class JVMWatcher {
	...
	public static void main(String[] args) {
		...
		System.out.println("INIT HEAP: " + toMSize(usage.getInit())); 
		System.out.println("MAX HEAP: " + toMSize(usage.getMax())); 
		System.out.println("USED HEAP: " + toMSize(usage.getUsed())); 
		System.out.println("\nFull Information:"); 
		...
		System.out.println("总的内存量 i is " + toMSize(Runtime.getRuntime().totalMemory()));
		int j = (int)Runtime.getRuntime().freeMemory()/1024;//Java 虚拟机中的空闲内存量
		System.out.println("空闲内存量 j is " + toMSize(Runtime.getRuntime().freeMemory()));
		System.out.println("最大内存量 is " + toMSize(Runtime.getRuntime().maxMemory()));
...
//	    System.out.println(osm.getTotalPhysicalMemorySize()/1024);
	...
	private static String toSize(Long value, String unit) {
		String ret = "";
		...
		case "K":
			...
			break;
		case "M":
			ret = value / (1024 * 1024) + unit;
			break;
		case "G":
			ret = value / (1024 * 1024 * 1024) + unit;
			break;
		}
		return ret;
	}

	private static String toMSize(Long value) {
		return toSize(value, "M");
	}
